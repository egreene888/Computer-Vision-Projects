...
class library(object):
    def __init__(self, size, filename, modelParams):
        self.size = size
    ...
    def makeModel(self):
        if self.model:
            return
        filename = "leefsnap_model.sklearn"
        # try:
        #     self.pickle.load(open(filename, 'rb'))
        # except IOError:
        sizeHiddenLayers = self.modelParams[0]
        alpha = self.modelParams[1]
        sizeHiddenLayers = 650 # number of species? It's guesswork.
        self.model = MLPClassifier(solver='adam', alpha=alpha,
...
class leaf(object):
    ...
    def findContours(self):
        ...
        self.contour = biggestContour.transpose().reshape((-1, 2)
            ).astype('int32')
        # print self.contour
        start = self.contour.argmax(axis = 0)[0]
        # print self.contour[start]
        # self.contour = np.append(self.contour[start:, :],
        #     self.contour[:start, :], axis = 0)
        indices = np.linspace(0, biggestContour.shape[1] - 1,
            NUM_POINTS).tolist()
        ...
        self.points = np.array([ [biggestContour[0][i], biggestContour[1][i] ]
            for i in indices])
        # print self.points
        start = self.points.argmax(axis = 0)[0]
        # print start
        self.points = np.append(self.points[start:, :],
            self.points[:start, :], axis = 0)
        # print self.points
        # self.showPoints()
...
def main():
    ...
    alpha = 1e-6
    NNSize = 200
    print "Hidden layer size = {}, alpha = {}".format(NNSize, alpha)
    params = [NNSize, alpha]
    lib = library(30866/2, 'leafsnap-dataset/leafsnap-dataset-images.txt',
        params)
    lib.test()
    """
    leaf1 = leaf(0, "canada.jpg", "Unknown")
    # print leaf1.contours()
    leaf1.findContours()
    # leaf1.showPoints()

    leaf2 = leaf(0, "canada2.jpg", "Unknown")
    leaf2.findContours()
    # leaf2.showPoints()
    """
    """
    ...
    for i in xrange(NUM_POINTS):
        pointx = int(leaf1.points[i][0] - leaf2.points[i][0])
        # print pointx
        pointy = int(leaf1.points[i][1] - leaf2.points[i][1])
        point = ((size + pointx, size + pointy))
        # point = point.reshape((-1, 2)).astype('int')
        # print point
        cv2.circle(img = canvas, center = point, radius = 2, color = 255)
    cv2.imshow('win', canvas)
    while cv2.waitKey() < 5: pass
    """
